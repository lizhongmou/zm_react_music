...
// 使用泛型，定义getstate的类型。在这里如果不想让getstate的类型不是unknow类型
// 需要使用这里传递的第三个参数。但是第三个参数继承与别的地方，因此使用的继承与别的地方的数据类型
...
export const changeMusicAction = createAsyncThunk<void, boolean, IThunkState>(
  ...
  (isNext, { dispatch, getState }) => {
    ...
    // 4.请求新的歌词，切换歌曲以后需要重新请求新的歌词
    getSongLyric(song.id).then((res) => {
      // 1.获取歌词的字符串
      const lyricString = res.lrc.lyric
      // 2.对歌词进行解析(一个个对象)
      const lyrics = parseLyric(lyricString)
      // 3.将歌词放到state中，转换以后的歌词
      dispatch(changeLyricsAction(lyrics))
    })
  }
)
